// backend/routes/bank_deposit.js — FINAL, SAFE & REAL-TIME READY
...
/* ============================
   POST /bank-deposit/log
   Log a new bank deposit
============================ */
router.post("/log", async (req, res) => {
  ...
  try {
    ...
    if (io) {
      io.emit("newTransaction", newDeposit);
      io.emit("dashboardUpdate", {
        service: "bank",
        amount: parsedAmount,
        action: "increment"
      });
      console.log("Bank deposit emitted to dashboard");
    }

    res.status(201).json({
      success: true,
      message: "Bank deposit logged successfully",
      transactionId,
      data: newDeposit
    });
  } catch (err) {
    ...
    res.status(500).json({
      success: false,
      message: err.sqlMessage || "Failed to save deposit"
    });
  }
});
